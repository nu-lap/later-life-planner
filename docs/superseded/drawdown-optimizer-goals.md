...
| Who calls MCP? | Developer, manually during session | No live LLP runtime call today | Audit route and explanation route only |
...
Layer 3 — LLM Explanation:
  [NOW: LLP route not built]────────[TARGET: fully dynamic, MCP + RAG]
...
| LLM explanation | No LLP explanation route yet | Build provider-agnostic API route with MCP citations and Cosmos retrieval first; upgrade retrieval or provider later if needed |
...
4. **Phase 4:** LLM explanation via `POST /api/optimizer-explain`. Plain-English rationale, concrete action list.
   Build the LLP API route to combine deterministic optimizer output, HMRC citations, and Cosmos-backed retrieval.
