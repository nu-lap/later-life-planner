...
This document defines the full set of goals a later-life drawdown optimiser should be capable of pursuing. Its primary purpose is to ensure the LaterLifePlan engine is architected for **flexibility from the outset** — so that users can express their own priority order across multiple goals rather than being locked into a single hard-coded objective (e.g. tax minimisation).

It complements:

- `docs/superseded/withdrawal-optimizer-mcp-design.md` — historical optimizer framing and solver design
- `docs/superseded/hmrc-tax-mcp-integration-plan.md` — historical HMRC rule integration
- `docs/optimizer-architecture-reconciled.md` — current cross-cutting architecture and implementation status
...
> LaterLifePlan is a tool to optimise later-life drawdown for *user-defined goals in user-defined priority order*. The engine computes; the user chooses what matters most.
...
- Spending categorised by tier: `essential | aspirational` (LaterLifePlan already has `tier` field in spending categories)
...
- LaterLifePlan already has a `careReserve` field in the plan model; this goal formalises its integration into the optimizer
...
| Metric | LaterLifePlan Waterfall | Combined Optimizer | Saving |
|---|---|---|---|
...
**Root cause of LaterLifePlan waterfall suboptimality:** Paul draws all above-PA DC by default. After SP starts (Paul 67), his taxable income reaches £52,424 — £2,154 above the £50,270 basic-rate limit — incurring 40% on the excess. Equal split keeps both partners at ~£34K taxable: fully basic-rate.
...
| | Historical proof of concept | Current implementation | Target |
|---|---|---|---|
| Source | Hardcoded constants in TypeScript | `gen-tax-snapshot.ts` → `taxRuleSnapshot.ts` committed in repo | hmrc-local MCP, called for audit and optionally for future regeneration hardening |
| Freshness | Stale until a developer manually updates | Regenerated by committed generator and checked for drift in CI | Versioned, citable, regenerated against rule-set changes |
| Who calls MCP? | Developer, manually during session | No live LaterLifePlan runtime call today | Audit route and explanation route only |
...
This is a firm design principle established in `docs/superseded/withdrawal-optimizer-mcp-design.md`:
...
> **Reconciled recommendation:** Keep the explanation layer provider-agnostic. Anthropic is acceptable for MVP because LaterLifePlan already has the SDK and streaming pattern. Azure OpenAI remains the preferred production target when quota and deployment are ready.
...
Layer 1 — Tax Rules:
  [NOW: committed snapshot]────[TARGET: build-time snapshot + live audit]

Layer 2 — Optimizer Core:
  [NOW: static scripts]──[TARGET: stays deterministic, rules from snapshot]

Layer 3 — LLM Explanation:
  [NOW: LaterLifePlan route not built]────────[TARGET: fully dynamic, MCP + RAG]
                                    (Cosmos retrieval first, API endpoint pending)
...
| Layer | Built | Gap |
|---|---|---|
| Tax rules | Snapshot integrated into planner runtime | Add optional live MCP audit route and later direct MCP-backed regeneration hardening |
| Optimizer core | Working in `scripts/combined-strategy.ts` | Needs porting to `src/financialEngine/withdrawalOptimizer.ts` |
| LLM explanation | No LaterLifePlan explanation route yet | Build provider-agnostic API route with MCP citations and Cosmos retrieval first; upgrade retrieval or provider later if needed |
| Goal orchestration | Not built | Needs goal registry + LLM orchestration layer |

The build-time snapshot substantially closes the Layer 1 gap. The remaining Layer 1 work is audit and regeneration hardening. Layers 3 and 4 remain the main product differentiation.

---

## Next Steps

1. **Phase 1:** Stabilise the HMRC tax snapshot path already in the codebase and keep `src/config/taxRuleSnapshot.ts` aligned with the generator and tests.
...
4. **Phase 4:** LLM explanation via `POST /api/optimizer-explain`. Plain-English rationale, concrete action list.
   Build the LaterLifePlan API route to combine deterministic optimizer output, HMRC citations, and Cosmos-backed retrieval.
