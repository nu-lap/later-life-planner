...
They are now represented in IaC and should be treated as deployment-managed resources.
...
Phase 1.5 IaC backfill is complete in `infra/main.bicep`:
...
The CI/CD deploy workflow now sets Cosmos DB and Key Vault runtime identifiers on each deploy.

The workflow also includes a post-deploy `persistence-smoke-test` job that validates:
