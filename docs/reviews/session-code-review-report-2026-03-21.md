...
| R12-P2 | P2 | Reliability | `src/lib/cosmos.ts` (`upsertDeviceRegistration`) | **Concurrent first-registration could 500**: `container.items.create` was not wrapped; concurrent first-create could bubble Cosmos 409/412 into a 500. | Fixed: handle 409/412, re-read and return existing device document (`cb1d15e`). |
| R13-P1 | P1 | Approval integrity | `src/app/api/devices/[deviceId]/approve/route.ts` + `src/hooks/usePlanSync.ts` | **Approver not verified**: any authenticated session could write a wrapped DEK package for a pending device, enabling approval races/DoS by non-approved sessions. | Fixed: require `approverDeviceId` and enforce approver is `status === 'active'`; added a bootstrap activation step and server endpoint (`/api/devices/activate`) (`9dd3d9e`). |
| R14-P2 | P2 | Testing | `tests/ui/usePlanSyncDeviceApproval.test.tsx` | **Missing deterministic AAD mismatch coverage**: recipient polling AAD mismatch was only indirectly exercised. | Fixed: added a deterministic UI test that asserts an AAD mismatch surfaces an error and does not silently proceed (`fe2e643`). |
| R15-P1 | P1 | Optimistic concurrency | `src/hooks/usePlanSync.ts` | **Revision conflict thrash**: on a 409 revision conflict the client would keep attempting debounced saves, potentially spamming requests and masking the conflict state. | Fixed: enter `conflict`, block further saves, and require explicit remote reload before continuing (`e211618`). |
| R16-M1 | MEDIUM | UX/conflict recovery | `src/app/page.tsx` + `src/components/Header.tsx` | **Conflict resolution not obvious**: conflict state was visible in the header but “Reload remote” lived on an account page, making it unclear what to do next. | Fixed: add a direct “Reload remote” action in the header during conflict (`f209fcc`), then add an in-page conflict toast with a primary reload action for iOS/keyboard scenarios (`cd85a8d`, `c843ea8`). |
| R17-M1 | MEDIUM | Mobile layout | `src/components/Header.tsx` | **iOS header crowding**: the save-status pill and “LifePlan” branding were too tightly spaced on small screens. | Fixed: stack header layout on mobile and keep controls right-aligned (`8cee3c6`). |
