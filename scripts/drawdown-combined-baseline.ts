...
const plan: PlannerState = {
  ...
  spendingCategories: base.spendingCategories.map((c: SpendingCategory) => ({
    ...c,
    amounts: {
      'go-go':  Math.round(c.amounts['go-go']  * 1.00),
      'slo-go': Math.round(c.amounts['slo-go'] * 1.00),
      'no-go':  Math.round(c.amounts['no-go']  * 1.00),
    },
  })),
};
...
console.log('\nBaseline  ·  Ages 60–69  ·  All figures nominal (2.5% inflation on spending)\n');
console.log(sep);
console.log(
  ' Ages   '.padEnd(9) +
