...
function AccountDevicesApprovePageWithClerk() {
  const [prefill, setPrefill] = useState<string>('');

  useEffect(() => {
    if (typeof window === 'undefined') return;
    setPrefill(window.location.href);
  }, []);
  ...
  return (
    <AccountLayoutShell saveStatus={sync.saveStatus}>
      <DeviceApprovalsPanel
        ...
        defaultApprovalInput={prefill}
      />
    </AccountLayoutShell>
  );
}

export default function AccountDevicesApprovePage() {
  if (!HAS_CLERK) {
    return (
  ...
  return <AccountDevicesApprovePageWithClerk />;
}
